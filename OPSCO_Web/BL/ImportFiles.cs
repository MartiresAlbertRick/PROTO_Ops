...
namespace OPSCO_Web
{
    ...
    { 
        ...
        public int ImportBIQualD(string filePath, Import import, DateTime dateUploaded, string user)
        {
            int result = 0;
            OleDbConnection con = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + filePath + ";Extended Properties='Excel 12.0;HDR=Yes;IMEX=6'");
            con.Open();
            ...
            OleDbDataReader dr = cmd.ExecuteReader();
            SqlBulkCopy bulkInsert = new SqlBulkCopy(sqlCon);
            bulkInsert.DestinationTableName = "OSC_ImportBIQualDetailed_temp";
            if (dr.HasRows)
            {
                dt.Load(dr);
                dt.Columns.Add("Month");
            ...
            if (result == 1)
            {
                ...
                result = this.ModifyDataStoredProcedure("OSC_ImportBIQualDetailed_Transfer", ArrayOfParameters);
            }
            #endregion "PushImport"
            return result;
        }
